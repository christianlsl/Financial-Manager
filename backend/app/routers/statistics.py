...
def get_detailed_statistics(
    ...
    profit_rate = float(sale_total) / float(purchase_total) - 1 if float(purchase_total) > 0 else 0
    ...
    if analysis_type == "monthly":
        ...
        ).filter(sale_base_filter).group_by(
        ...
        for trend in purchase_trend:
            ...
            trend_data[key] = {
                'purchase': float(trend.purchase_amount),
                'sale': 0.0
            }
    ...
    # 3. 获取分类对比数据（按业务类型）
    purchase_by_type = db.query(
        Purchase.type,
        func.coalesce(func.sum(Purchase.total_price), 0).label('amount')
    ).filter(base_filter).group_by(Purchase.type).all()
    
    sale_by_type = db.query(
        Sale.type,
        func.coalesce(func.sum(Sale.total_price), 0).label('amount')
    ).filter(sale_base_filter).group_by(Sale.type).all()
    ...
    if analysis_type == "yearly":
        ...
        customer_sales = db.query(
            ...
            extract('year', Sale.date).label('year'),
            func.coalesce(func.sum(Sale.total_price), 0).label('sale_amount')
        ).select_from(Sale).join(Customer, Sale.customer_id == Customer.id).filter(
            sale_base_filter
        ).group_by(
            Customer.name,
            extract('year', Sale.date)
        ).order_by('year', 'customer_name').all()
        
        # 获取所有年份
        years_query = db.query(
            extract('year', Sale.date).distinct().label('year')
        ).filter(sale_base_filter).order_by('year').all()
        ...
        customer_sales = db.query(
            ...
            extract('year', Sale.date),
            extract('month', Sale.date)
        ).order_by('year', 'month', 'customer_name').all()
        
        # 获取所有月份
        months_query = db.query(
            extract('year', Sale.date).label('year'),
            extract('month', Sale.date).label('month')
        ).filter(sale_base_filter).group_by('year', 'month').order_by('year', 'month').all()
