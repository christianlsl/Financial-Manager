...
def test_update_sale_image_via_multipart_triggers_delete(client, auth_headers, attach_vendor, monkeypatch):
    headers = auth_headers("owner@example.com")
    company_resp = client.post(
        "/companies/",
        json={"name": "UploaderCo", "address": "Industrial Park"},
        headers=headers,
    )
    assert company_resp.status_code == 200, company_resp.text
    company_id = company_resp.json()["id"]
    ...
    customer_resp = client.post(
        "/customers/",
        json={"name": "Uploader Customer", "company_id": company_id},
        headers=headers,
    )
    assert customer_resp.status_code == 200, customer_resp.text
    customer_id = customer_resp.json()["id"]
    ...
    old_url = "https://cdn.example.com/sales-images/old-image.jpg"
    ...
    def fake_upload(data: bytes, filename: str | None = None) -> str:  # pragma: no cover - test helper
        ...
        return "https://cdn.example.com/sales-images/new-image.jpg"

    def fake_delete(url: str | None) -> None:  # pragma: no cover - test helper
        deletes.append(url or "")

    monkeypatch.setattr("app.routers.sales.uploader.upload", fake_upload)
    monkeypatch.setattr("app.routers.sales.uploader.delete", fake_delete)
    ...
    assert update_resp.json()["image_url"] == "https://cdn.example.com/sales-images/new-image.jpg"
